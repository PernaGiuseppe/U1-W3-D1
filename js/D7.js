...
const concatenazione = (stringa1, stringa2) => {
  const risultato = stringa1.slice(0, 2) + stringa2.slice(-3)
  const risultato1 = risultato.toUpperCase()
  return risultato1
}

const risultato1 = concatenazione('Topo', 'Gigione')
...
const calcolatore = () => {
  const arrayDaCreare = []
  const numeroArray = 10
  for (let i = 0; i < numeroArray; i++) {
    const numrandom = Math.floor(Math.random() * (100 - 10))
    arrayDaCreare.push(numrandom)
  }
  return arrayDaCreare
}
const array = calcolatore()
console.log(array)
...
const arrayDiNumeri = [10, 33, 46, 58, 67, 21, 88, 99]
const numeriPari = arrayDiNumeri.filter((numero) => {
  if (numero % 2 === 0) {
    return true
  } else {
    return false
